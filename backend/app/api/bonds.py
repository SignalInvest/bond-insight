...
def get_bonds(
    ...
    issuer: str | None = Query(None, min_length=1, max_length=100),
    ...
    if maturity_from is not None and maturity_to is not None and maturity_from > maturity_to:
        raise HTTPException(status_code=422, detail="maturity_from must be <= maturity_to")
    ...
    try:
        return list_bonds(
            db,
            page=page,
            page_size=size if size is not None else page_size,
            search=search,
            issuer=issuer,
            bond_type=bond_type,
            rating=rating,
            ...
            maturity_from=maturity_from.isoformat() if maturity_from else None,
            maturity_to=maturity_to.isoformat() if maturity_to else None,
            sort_by=sort_by,
        ...
        raise HTTPException(
            status_code=503,
            detail="Bond data service is temporarily unavailable"
        ) from e
